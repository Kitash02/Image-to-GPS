...
def main():
    ...
    # Find input root + all subfolders containing images, ignore hidden folders/files
    candidate_dirs = [input_dir] + [p for p in sorted(input_dir.rglob("*")) if p.is_dir()]
    for subfolder in candidate_dirs:
    ...
    for img_path, prefix in images:
        ...
        # Save exact relative path for unambiguous dataset loading.
        data["relative_path"] = str(img_path.relative_to(input_dir))
        rows.append(data)
    
    # Create DataFrame with core fields + relative path for deterministic loading.
    df = pd.DataFrame(rows, columns=["image_name", "Latitude", "Longitude", "relative_path"])
    ...
    print(f"[OK] Saved {len(df)} entries to: {out_path.resolve()}")
    print(f"\nDataset Statistics:")
