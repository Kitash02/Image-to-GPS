...
class GPSRegressionModel(nn.Module):
    ...
    BACKBONE_CONFIGS = {
        "resnet50": {
            "model_fn": lambda use_pretrained: models.resnet50(weights="DEFAULT" if use_pretrained else None),
            "output_features": 2048,
            "input_size": 224,
        },
        "resnet101": {
            "model_fn": lambda use_pretrained: models.resnet101(weights="DEFAULT" if use_pretrained else None),
            "output_features": 2048,
            "input_size": 224,
        },
        "efficientnet_b0": {
            "model_fn": lambda use_pretrained: models.efficientnet_b0(weights="DEFAULT" if use_pretrained else None),
            "output_features": 1280,
            "input_size": 224,
        },
        "efficientnet_b1": {
            "model_fn": lambda use_pretrained: models.efficientnet_b1(weights="DEFAULT" if use_pretrained else None),
            "output_features": 1280,
            "input_size": 240,
        },
        "convnext_tiny": {
            "model_fn": lambda use_pretrained: models.convnext_tiny(weights="DEFAULT" if use_pretrained else None),
            "output_features": 768,
            "input_size": 224,
        },
        "vit_b16": {
            "model_fn": lambda use_pretrained: models.vit_b_16(weights="DEFAULT" if use_pretrained else None),
            "output_features": 768,
            "input_size": 224,
        },
    }
    
    def __init__(self, backbone_name: str = "resnet50", dropout_rate: float = 0.3, use_pretrained: bool = True):
        """
        Args:
        ...
        # Load backbone (ImageNet-pretrained or randomly initialized)
        self.backbone = config["model_fn"](use_pretrained)
